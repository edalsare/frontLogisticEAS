...
export class ProductoComponent implements OnInit {
  ...
  onSelect() {
    this._sercCategoria
      .getBynamecat(this.formProducto.get('categoría')?.value)
      .subscribe({
        next: (data) => {
          this.categoria = data;
        },
  ...
  enviar() {
    ...
    this._productoService
      .getProductExist(this.formProducto.get('ean')?.value)
      .subscribe({
        next: (data) => {
          if (data == true) {
            this.banFail = true;
            this.mensaje =
              'Este producto ya se encuentra Registrado, prueba otro EAN';
          } else {
            this.banFail = false;
            this.mensaje = '';
            for (let pro of this.product) {
              if (pro.idproduct == producto.idproduct) {
                this.banFail = true;
                this.mensaje =
                  'Este producto ya se encuentra en la lista a registrar';
              }
            }
